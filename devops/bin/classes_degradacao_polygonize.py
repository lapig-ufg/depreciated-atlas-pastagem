...
def polygonize(input_feature, input_feature_srs, input_feature_def, input_value_raster, fid):
		timestart = time.time()
		...
		out_lyr = out_ds.GetLayerByName('classes_degradacao_pastagem')

		if out_lyr is None:
			out_lyr = out_ds.CreateLayer('classes_degradacao_pastagem', srs = raster_srs, geom_type=ogr.wkbMultiPolygon)
...
def feature_loop(input_zone_polygon):
		...
		def parallelProcess(input_zone_polygon, FID):
			...
			input_value_raster = '/data/containers/POSTGIS/APP/temporario/Classes_degradacao/Pasture_Quality_Brasil_2018.tif'
			shp = ogr.Open(input_zone_polygon)
			vector_layer = shp.GetLayer()
			...
			try:
				polygonize(input_feature, input_feature_srs, input_feature_def, input_value_raster, FID)
			except Exception:
				traceback.print_exc()
